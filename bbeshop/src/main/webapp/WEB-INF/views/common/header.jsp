...
<script type="text/javascript">
	...
	function keywordSearch() {
		...
		if (searchWordValue.length > 0) {
			$.ajax({
				...
				data : {
					keyword : searchWordValue
				// 검색어 값을 'keyword' 파라미터로 전달합니다.
				},
    ...
    const goodsList = document.querySelectorAll('.goods-item');
    ...
    // 카테고리에 해당하는 상품만 보이도록 필터링
    function filterGoodsByCategory(category) {
        goodsList.forEach(item => {
            if (item.dataset.category === category) {
                item.style.display = 'block'; // 해당 카테고리의 상품은 보이도록 설정
            } else {
                item.style.display = 'none';  // 다른 카테고리의 상품은 숨기도록 설정
            }
        });
    }
</script>
			...
			<div class="dropdown">
				...
				<ul class="dropdown-menu text-small shadow">
					<li><a class="dropdown-item small"
						href="${contextPath}/goods/menuGoods.do">전체상품</a></li>
					<li><a class="dropdown-item small"
						href="${contextPath}/goods/menuGoods.do?menuGoods=와인잔">와인잔</a></li>

					<li><a class="dropdown-item small"
						href="${contextPath}/goods/menuGoods.do?menuGoods=맥주잔">맥주잔</a></li>
					<li><a class="dropdown-item small"
						href="${contextPath}/goods/menuGoods.do?menuGoods=캐릭터잔">캐릭터잔</a></li>
					<li><a class="dropdown-item small"
										...
										<div class="dropdown-menu text-small shadow">
											<li><a href="${contextPath}/cart/myCartList.do"
												class="text-decoration-none"> <span
													class="dropdown-item">${memberInfo.memberName}</span></a></li>
											<li><a
